...
def add_commodity(request):
    ...
    if request.method == "POST":
        # Check if it's a custom commodity
        if 'custom_commodity' in request.POST and request.POST['custom_commodity']:
            # Create new custom commodity
            custom_name = request.POST['custom_commodity'].strip()
            if custom_name:
                # Check if commodity already exists (case-insensitive)
                existing = ListCommodity.objects.filter(name__iexact=custom_name).first()
                if existing:
                    commodity = existing
                else:
                    commodity = ListCommodity.objects.create(
                        name=custom_name,
                        is_custom=True,
    ...
    # For GET requests
    commodities = ListCommodity.objects.filter(is_custom=False)  # Show only non-custom by default
    return render(request, 'your_template.html', {'commodities': commodities})
@login_required
...
from .forms import CustomUserCreationForm, AcceptOrderForm  # Import the new form
...
def supplier_orders(request):
    ...
    # Pending orders with vendor address
    pending_orders = Order.objects.filter(
        supplier_commodity__supplier=request.user,
        status="pending"
    ).select_related('vendor').order_by('-ordered_at')

    # Previous orders with time filtering
    time_filter = request.GET.get('time_filter', 'all')
    previous_orders = Order.objects.filter(
        supplier_commodity__supplier=request.user
    ).select_related('vendor').exclude(status="pending")
...
def accept_order(request, order_id):
    """Supplier accepts an order with delivery details."""
    ...
    if request.method == 'POST':
        form = AcceptOrderForm(request.POST, instance=order)
        if form.is_valid():
            if order.quantity_requested > order.supplier_commodity.available_units:
                ...
                order.delivery_address = f"{order.vendor.address}, {order.vendor.pincode}, {order.vendor.state}"
                order.save()
                order.supplier_commodity.available_units -= order.quantity_requested
                order.supplier_commodity.save()
                messages.success(request, "Order accepted with delivery details.")
                return redirect("supplier_orders")
    else:
        form = AcceptOrderForm(instance=order)

    return render(request, "inventory/accept_order.html", {
        'order': order,
        'form': form
    })
@login_required
